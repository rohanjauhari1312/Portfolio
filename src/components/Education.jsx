...
function EducationCard({ e, idx, visible, isMobile }) {
  ...
  return (
    <div
      ...
      style={{
        opacity: visible ? 1 : 0,
        transform: !visible ? 'translateY(20px)' : hovered ? 'translateY(-8px)' : 'translateY(0)',
        transition: !visible
          ? `opacity 0.5s ease ${idx * 120}ms, transform 0.6s cubic-bezier(.22,1,.36,1) ${idx * 120}ms`
    ...
    ><div
      style={{
        ...
        boxShadow: hovered
          ? `0 0 24px ${e.color}80, 0 0 60px ${e.color}45, 0 0 100px ${e.color}20, inset 0 0 0 1px ${e.color}30`
          : `0 0 18px ${e.color}18, inset 0 0 0 1px ${e.color}10`,
    ...
    <div id="education" style={{ background: 'rgba(10,10,10,0.85)', padding: isMobile ? '24px 0 48px' : '40px 0 80px' }}>
      ...
      <div ref={ref} style={{ maxWidth: 1320, margin: '0 auto', padding: isMobile ? '0 24px' : '0 64px' }}>
        ...
        <div style={{
          ...
          <TypedHeading text="My academic " suffixText="foundation." suffixStyle={{ background: 'linear-gradient(90deg, #facc15, #fb923c)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text' }} style={{
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
          {EDUCATION.map((e, idx) => (
            <EducationCard key={e.school} e={e} idx={idx} visible={visible} isMobile={isMobile} />
          ))}
        </div>
      </div>
    </div>
